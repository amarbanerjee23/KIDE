...
export const ArtifactOutline: React.FC = () => {
  ...
  if (!activeFile) {
    return (
      <div className="flex flex-col items-center justify-center p-6 text-center text-gray-500 h-full select-none">
        <ListTree className="w-7 h-7 text-gray-600 mb-2" />
        <p className="text-xs font-medium text-gray-300">No Artifact Selected</p>
        <p className="text-[11px] text-gray-500 mt-1">Select an artifact to inspect its outline structure.</p>
      </div>
    );
  }
  ...
  if (symbols.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-6 text-center text-gray-500 h-full select-none">
        <ListTree className="w-7 h-7 text-gray-600 mb-2" />
        <p className="text-xs font-medium text-gray-300">No Semantic Symbols</p>
        <p className="text-[11px] text-gray-500 mt-1">No declared models or operations found in {activeFile.name}.</p>
      </div>
    );
  }
    ...
    return (
      <div key={node.id} className="select-none">
        <div
          ...
          className="group flex items-center justify-between py-1.5 pr-2 hover:bg-gray-800/60 rounded cursor-pointer text-xs text-gray-300 hover:text-white transition-colors"
        >
