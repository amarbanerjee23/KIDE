...
export const exportRos2 = (model: MncModel) => exportFormat('ros2', model);
export const exportJava = (model: MncModel) => exportFormat('java', model);
export const exportPlc = (model: MncModel) => exportFormat('plc', model);
export const exportCpp = (model: MncModel) => exportFormat('cpp', model);
...
export const downloadZipBundle = async (model: MncModel, filename: string = 'supervisor_bundle.zip'): Promise<void> => {
  try {
    ...
    console.error('Failed to download ZIP bundle:', error);
    throw error;
  }
};
