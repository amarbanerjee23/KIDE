#!/usr/bin/env python3
"""Run the PR22 secure WebSocket/LSP self-check from packaged headless KIDE bytes."""
...
MARKER = "KIDE PR22 SECURE LSP WEBSOCKET SELF-CHECK OK"
...
def run_selfcheck(launcher: Path) -> None:
    result = subprocess.run(
        [
            ...
            "-consoleLog",
            "-application",
            "com.kide.languageserver.gateway.selfcheck",
        ],
        cwd=launcher.parent,
        env=os.environ.copy(),
        text=True,
    ...
    if result.returncode != 0:
        raise SmokeFailure(
            f"packaged PR22 gateway self-check exited {result.returncode}\n{output[-10000:]}"
        )
    if MARKER not in output:
        raise SmokeFailure(
            f"packaged PR22 gateway success marker missing\n{output[-10000:]}"
        )
    print(f"PR22 PACKAGED SECURE LSP WEBSOCKET QUALIFIED: {launcher}")
