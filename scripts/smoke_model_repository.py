...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="kide-pr21-runtime-") as td:
        root=Path(td); safe_extract(archive,root)
        launcher=find_launcher(root); launcher.chmod(launcher.stat().st_mode|0o111)
        workspace=root/"workspace"; workspace.mkdir()
        result=subprocess.run([str(launcher),"-nosplash","-consoleLog","-application","com.kide.enterprise.modelrepo.selfcheck","-data",str(workspace)],
        ...
        marker="KIDE PR21 MODEL REPOSITORY SELF-CHECK OK"
        if result.returncode!=0 or marker not in output:
            raise SmokeError(f"packaged PR21 model repository self-check failed ({result.returncode})\n{output[-6000:]}")
        print(f"PR21 PACKAGED MODEL REPOSITORY QUALIFIED: {archive.name}")
    return 0
