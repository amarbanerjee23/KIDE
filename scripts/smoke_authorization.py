#!/usr/bin/env python3
"""Run the PR18 authorization self-check from packaged Linux KIDE bytes."""
...
def run_selfcheck(archive: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="kide-pr18-runtime-") as temp_dir:
        root = Path(temp_dir)
        ...
        result = subprocess.run(
            [
                ...
                "-nosplash",
                "-consoleLog",
                "-application", "com.kide.enterprise.authorization.selfcheck",
                "-data", str(workspace),
            ],
            cwd=launcher.parent,
            env=os.environ.copy(),
        ...
        marker = "KIDE PR18 AUTHORIZATION SELF-CHECK OK"
        if result.returncode != 0:
            raise SmokeError(
                f"packaged PR18 authorization self-check exited {result.returncode}\n{output[-6000:]}"
            )
        if marker not in output:
            raise SmokeError(
                f"packaged PR18 authorization success marker missing\n{output[-6000:]}"
            )
        print(f"PR18 PACKAGED AUTHORIZATION RUNTIME QUALIFIED: {archive.name}")
