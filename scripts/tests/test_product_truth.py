...
class ProductTruthTest(unittest.TestCase):
    ...
    def test_registry_is_exactly_six_unique_production_languages(self):
        languages = verify_product_truth.load_registry(ROOT)
        self.assertEqual(6, len(languages))
        self.assertEqual(6, len({item["extension"] for item in languages}))
        self.assertEqual(6, len({item["language_id"] for item in languages}))
