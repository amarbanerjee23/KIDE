#!/usr/bin/env python3
"""Run the PR19 audit self-check from packaged Linux KIDE bytes."""
...
def run_selfcheck(archive: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="kide-pr19-runtime-") as temp_dir:
        root = Path(temp_dir)
        ...
        result = subprocess.run(
            [
                ...
                "-nosplash",
                "-consoleLog",
                "-application", "com.kide.enterprise.audit.selfcheck",
                "-data", str(workspace),
            ],
            cwd=launcher.parent,
            env=os.environ.copy(),
        ...
        marker = "KIDE PR19 AUDIT SELF-CHECK OK"
        if result.returncode != 0:
            raise SmokeError(
                f"packaged PR19 audit self-check exited {result.returncode}\n{output[-6000:]}"
            )
        if marker not in output:
            raise SmokeError(f"packaged PR19 audit success marker missing\n{output[-6000:]}")
        print(f"PR19 PACKAGED AUDIT RUNTIME QUALIFIED: {archive.name}")
