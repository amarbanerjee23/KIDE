...
package com.system.knowledge.engine.ocl;
...
//        JavaPackage.eINSTANCE.eClass();
        ...
        try (PersistentResource resource = (PersistentResource) resourceSet.createResource(new BlueprintsUriFactory().createLocalUri("databases/sample2.graphdb"))) {
...
//            List<MethodDeclaration> result = getProtectedMethodDeclarations(resource);
            stopwatch.stop();
//            System.out.println("[ProtectedMethods - GraphDB] Done, found "+result.size()+" elements in "+stopwatch.elapsed().getSeconds()+" seconds");
        }
    }

	/*
	 * @SuppressWarnings("unchecked") private static List<MethodDeclaration>
	 * getProtectedMethodDeclarations(Resource resource) { try { OCL ocl =
	 * OCL.newInstance(EcoreEnvironmentFactory.INSTANCE); OCLInput oclInput = new
	 * OCLInput(new FileInputStream(new File("ocl/protectedMethods.ocl")));
	 * List<Constraint> constraints = ocl.parse(oclInput); return
	 * (List<MethodDeclaration>)
	 * ocl.createQuery(constraints.get(0)).evaluate(resource.getContents().get(0));
