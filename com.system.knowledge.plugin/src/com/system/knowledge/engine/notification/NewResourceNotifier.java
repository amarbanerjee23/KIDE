...
package com.system.knowledge.engine.notification;
...
//            resource.getContents().add(newModel);
			 ...
			 * Log.info("Notification (type {0}) received from {1}", msg.getEventType(),
			 * msg.getNotifier().getClass().getName()); } });
			 * 
			 * // Update the Model String oldName = newModel.getName(); // Doesn't generate
			 * a log newModel.setName("New Name"); // Generates a log
			 * newModel.getCompilationUnits().add(factory.createCompilationUnit()); //
			 * Generates a log
			 * 
			 * // Save the resource and update it again
			 * resource.save(Collections.emptyMap()); newModel.setName(oldName);
			 */                                    // Generates a log
        }
    }
}
