...
gcloud projects add-iam-policy-binding "${PROJECT_ID}"   --member="serviceAccount:${BUILD_SERVICE_ACCOUNT}"   --role="roles/run.admin" >/dev/null

for service_account in "${RUNTIME_SA}" "${WEB_RUNTIME_SA}"; do
  gcloud iam service-accounts add-iam-policy-binding "${service_account}"     --project "${PROJECT_ID}"     --member="serviceAccount:${BUILD_SERVICE_ACCOUNT}"     --role="roles/iam.serviceAccountUser" >/dev/null
done
